...
function Navigation(props) {
  ...
  return (
    <div
      className={`navigation ${style === "dark" ? "navigation_dark" : ""} ${isActive ? "navigation_active" : "navigation_inactive"}`}>
      <nav className="navigation__links">
        ...
        {isLoggedIn ? <NavLink exact to="/saved-news"
                               activeClassName={style === "dark" ? "navigation__link_dark_active" : "navigation__link_light_active"}
      ...
      {isLoggedIn ?
        ...
        :
        <button
          className={`navigation__button ${style === "dark" && !isActive ? "navigation__button_dark" : "navigation__button_light"}`}>Авторизоваться</button>}
    </div>
  );
}
