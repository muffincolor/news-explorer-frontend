...
function Register(props) {
  const { onClose, onRegister, openLoginPopup, registerMessageError, setRegisterMessageError } = props;
  ...
  return (
    <PopupWithForm formReference={formReference} onSubmit={handleSubmit} closePopupsByButtons={onClose} title="Регистрация">
      ...
      <input value={values.email || ""} onChange={handleInputChange} required placeholder="Введите почту" id="email" name="email" type="email" className="popup__input"/>
      <span className="popup__error">{errors.email}</span>
      <label htmlFor="password" className="popup__label">Пароль</label>
      <input value={values.password || ""} onChange={handleInputChange} required placeholder="Введите пароль" id="password" name="password" type="password" className="popup__input"/>
      <span className="popup__error">{errors.password}</span>
      <label htmlFor="name" className="popup__label">Имя</label>
      <input value={values.name || ""} onChange={handleInputChange} required placeholder="Введите своё имя" id="name" name="name" type="text" className="popup__input"/>
